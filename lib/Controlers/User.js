...
exports.checkAdmin = (req, res, next) => {
    ...
    user.getUserByToken(token.token)
        .then(results => {
            ...
            if (data.length && data[0].user_type > 1) {
                req.body.user = data[0];
                next();
            } else {
                res.cookie('userinfo', '');
                res.json({code: '20000', msg: '用户不存在或已过期，请重新登录'});
            }
        })
        .catch(err => {
            res.json({err: err});
...
exports.checkUserName = (req, res, next) => {
    ...
    user.getUserByUsername(username)
        .then(results => {
            let data =  JSON.parse(JSON.stringify(results));
            if (data.length) {
                res.json({code: '20000', msg: '该用户名已被注册'});
            } else {
                next();
            }
        })
        .catch(err => {
            res.json({code: '20000', msg: '注册失败!'});
        })
}
...
exports.checkEmail = (req, res, next) => {
    ...
    user.getUserByEmail(email)
        .then(results => {
            let data =  JSON.parse(JSON.stringify(results));
            if (data.length) {
                res.json({code: '20000', msg: '该邮箱已被注册'});
            } else {
                next();
            }
        })
...
exports.regist = (req, res, next) => {
    let avatar = req.body.avatar || 'https://avatars2.githubusercontent.com/u/20011131';
    let username = req.body.username;
    let email = req.body.email;
    ...
    md5.update(password);
    password = md5.digest('hex');
    let user = new User(email, username, password, avatar, moment(Date.now()).format('YYYY-MM-DD HH:mm:ss'), user_type, 0);
    user.regist()
        .then(results => {
            res.json({code: '10000', msg: '注册成功!'})
        })
        .catch(err => {
            res.json({code: '20000', msg: '注册失败!', err: err});
        })
}